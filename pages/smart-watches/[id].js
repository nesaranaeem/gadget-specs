...
import { allGadgets, smartWatches } from "@/utils/api";
...
export default function LatestGadgets() {
  ...
  const { data, error, isLoading } = useSWR(
    `${smartWatches}page=${currentPage}&limit=${itemsPerPage}`,
    fetcher
  );
  ...
  const handlePageChange = (pageNumber) => {
    router.push(`/smart-watches/${pageNumber}`);
    setCurrentPage(pageNumber);
  };

  const handleItemsPerPageChange = (event) => {
    setItemsPerPage(parseInt(event.target.value));
    router.push(`/smart-watches/${currentPage}`);
  };
